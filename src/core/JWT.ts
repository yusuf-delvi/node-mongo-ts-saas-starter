...
async function encode(payload: JwtPayload): Promise<string> {
	...
	// @ts-ignore
	return promisify(sign)({ ...payload }, jwtSecret);
}
...
async function validate(
	...
	try {
		// @ts-ignore
		return (await promisify(verify)(token, jwtSecret)) as JwtPayload;
	} catch (e: any) {
		Logger.debug(e);
		if (e && e.name === 'TokenExpiredError') {
			if (isRefresh) throw new RefreshTokenExpiredError();
...
async function decode(token: string): Promise<JwtPayload> {
	...
	try {
		// @ts-ignore
		return (await promisify(verify)(token, jwtSecret, {
